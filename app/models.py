...
class MerchantStaff(Base):
    ...
    merchant_branch = Column(Integer(), ForeignKey(column="merchant_branches.id", ondelete="CASCADE"))
    status = Column(Enum(Status), nullable=False)
    created_by= Column(DateTime(timezone=True), nullable=False, server_default=text('now()'))
